...
RUN_NAME="local_control_high_chair_bb_backrest"
CONTROL_SUPERQUADRIC_PATH="examples/superquadrics/high_chair/high_chair.npz"
LOW_CONTROL_SUPERQUADRIC_MASK_PATH="examples/superquadrics/high_chair/bb_back_rest.npz"
...
srun --ntasks=1 --export=ALL \
  ...
  --output_dir "outputs/$RUN_NAME/high_control_10_low_control_3_low_control_mask_0_325" \
...
srun --ntasks=1 --export=ALL \
  ...
  --guidance_mode similarity \
  --appearance_image "$APPEARANCE_IMAGE" \
  --output_dir "outputs/$RUN_NAME/high_control_10_low_control_3_low_control_mask_0_375" \
  --shape_superquadric_path "$CONTROL_SUPERQUADRIC_PATH" \
  --shape_tau 3.0 \
  --convert_yup_to_zup \
  --low_control_superquadric_mask_path "$LOW_CONTROL_SUPERQUADRIC_MASK_PATH" \
