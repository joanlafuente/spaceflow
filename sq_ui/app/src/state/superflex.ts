...
function superflexBase(): string {
  return serviceBaseUrl(import.meta.env.VITE_SUPERFLEX_URL, 'http://localhost:11436');
}

export interface SuperflexGenerateOptions {
  file: File;
  name?: string;
  zUp?: boolean;
  normalize?: boolean;
...
export interface SuperflexGenerateResult {
...
interface SuperflexGenerateResponse {
  status: 'ok';
  run_id: string;
...
function resolveUrl(pathOrUrl: string): string {
  ...
  const base = superflexBase().replace(/\/$/, '');
...
async function parseJson<T>(res: Response): Promise<T> {
  if (!res.ok) {
    let msg = `SuperFlex returned ${res.status}`;
    try {
    ...
    if (looksLikeSlurmAlloc) {
      msg +=
        `\n\n` +
        `This is a Slurm allocation error. Fix by setting env vars for the SuperFlex service, e.g.` +
        `\n  SQ_SUPERFLEX_SLURM_PARTITION=<your_partition>` +
        `\n  SQ_SUPERFLEX_SLURM_ACCOUNT=<your_account>` +
        `\n(or run SuperFlex on a GPU node and set SQ_SUPERFLEX_FORCE_LOCAL=1 to disable srun wrapping).`;
    }
    throw new Error(msg);
  }
...
export async function generateWithSuperflex(
  options: SuperflexGenerateOptions
): Promise<SuperflexGenerateResult> {
  ...
  const res = await fetch(resolveUrl('/superflex/generate'), {
    method: 'POST',
    body: form,
  });
  const payload = await parseJson<SuperflexGenerateResponse>(res);

  const npzRes = await fetch(resolveUrl(payload.download_url));
  if (!npzRes.ok) {
    const text = await npzRes.text().catch(() => '');
    throw new Error(`Could not download SuperFlex output: ${npzRes.status} ${text.slice(0, 200)}`);
  }
  const blob = await npzRes.blob();
  const primitives = await importNpzToPrimitives(blob, options.name || options.file.name.replace(/\.[^.]+$/, '') || 'superflex');
